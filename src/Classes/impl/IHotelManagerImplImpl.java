 ...
 * <em><b>IHotel Manager Impl</b></em>'. <!-- end-user-doc -->
 * <p>
 ...
 * @generated
 */
public class IHotelManagerImplImpl extends MinimalEObjectImpl.Container
	...
	protected IHotelManagerImplImpl() {
		...
		StaffMember newStaffMember = ClassesFactoryImpl.eINSTANCE
				.createStaffMember();
	 ...
	 * @generated NOT
	 */
	public boolean addStaffMember(String adminUsername, String username,
	...
	public boolean isValidUsername(String username) {
		for (char ch : username.toCharArray()) {
			if (!((48 <= ch && ch <= 57) || (65 <= ch && ch <= 90) || (97 <= ch && ch <= 122))) {
				...
				// characters that are not ASCII letters nor digits are not
				// allowed:
				return false;
			}
		}
	 ...
	 * room(s) is Available and the current date is the same as the check-in
	 * date. This method is used to check in to all room(s) in a booking at the
	 * same time. If a room in the booking has already been checked in to, it is
	 * just ignored.
	 * 
	 * @generated NOT
	 */
	public String checkInBooking(int bookingID, String staffMemberUsername) {
		...
		EList<Booking> bookings = getIBookingManagementImpl()
		...
		// Set parameters of currentDate to the check-in time of the date for
		// comparison
		currentDate.set(Calendar.YEAR, Calendar.MONTH, Calendar.DAY_OF_MONTH,
				Calendar.HOUR, 00);
		...
		if (calTest.get(0) == currentDate.get(0)
			...
			for (Room room : rooms) {
				...
				if (room.getStatus() == RoomStatus.AVAILABLE) {
					...
					roomCheckIns++;
					roomNumbersSuccess.add(roomNumbersFail
							.remove(roomNumbersFail.size() - 1));
				}
			}
			if (roomCheckIns == rooms.size()) {
				return "Checked in successfully to rooms "
						+ roomNumbersSuccess.toString();
			} else {
				return "Failed to check in to rooms "
						+ roomNumbersFail.toString();
			}
		}
	...
	/**
	 * Change status of a room. This method is used by Staff when they need to
	 * change the status of a room outside of the check-in/check-out (and other
	 * defined) processes, for example if a room need to be unavailable for a
	 * while (perhaps due to maintenance).
	 * 
	 * @generated NOT
	 */
	public String changeStatusOfRoom(int roomID, String status,
			String staffMemberUsername) {
	 ...
	 * 
	 * Checks if the bill of the given booking is paid for (if all charges are
	 * set to 0). If the bill is paid, the customer is checked-out and the
	 * room(s) status is set to Cleaning as a default since the room would
	...
	public String checkOut(int bookingID, String staffMemberUsername) {
		...
		currentTime.set(Calendar.YEAR, Calendar.MONTH, Calendar.DAY_OF_MONTH,
				Calendar.HOUR, 00);
		...
		for (Booking booking : bookings) {
			if (booking.getBookingID() == bookingID) {
				...
				EList<Room> rooms = booking.getRooms();
				for (Room room : rooms) {
					// If a room is not Occupied (checked-in), a check out
					// cannot be performed
				...
				if (currentTime.after(testTime)) {
					for (Charge aCharge : charges) {
						// Check if the Late Check Out Fee exists and if it has
						// been paid
					...
					// If check-out time is later than bookings check-out time
					// and there are
					// no prior LateCheckOutFee charge present in the bill; add
					// the charge
					if (!feeExists) {
	...
	/**
	 * Adds a room with the specified room type and room number if the user is
	 * an admin.
	 * 
	 * @generated NOT
	 */
...
<<<<<<< HEAD
				//H‰r fˆrsˆker jag komma vilken roomType jag skall tilldela rummet
				RoomType r;
				room.setRoomType(r);
=======
